...
public:
    ...
    MyPoint(int x, int y, float z) : x_{x}, y_{y}, z_{z} { } //by the time the constructor completes, all member variables must be initialized

    // Methods that do not modify member variables should be declared const.
    void print_coords() const {
        cout << "(x, y, z) = (" << x_ << ", " << y_ << ", " << fixed //RELEVANT FOR LAB?
    ...
    // Accessor.
    int get_x() const {			//the order in which the member variables are declared, is the order in which they must be initialized
        return x_;
    }
...
int main() {
    ...
    MyPoint point1(5, 7, 1.9823412);  //these are VARIABLES, make them like this
    MyPoint point2(1, 2, 3);
    MyPoint point3(4, 5, 6);

    vector<MyPoint> points;
    points.push_back(point1);		//always get a copy when putting into vectors
